...
def show_district_reports(df_raw):
    ...
    if districts:
        ...
        if district_data.empty:
            ...
            # ✅ عنوان تسويقي ديناميكي للتقرير (يظهر قبل زر إنشاء التقرير)
            report_titles = {
                "📈 مؤشر اتجاه الأسعار": f"هل أسعار {property_type} في حي {district} ترتفع أم تنخفض؟",
                "💰 فرص الاستثمار": f"هل الاستثمار في {property_type} في حي {district} مربح في الوقت الحالي؟",
                "⚖️ المقارنة الذكية": f"كيف يقارن حي {district} مع الأحياء الأخرى في {city}؟",
                "🏘️ دليل السكن": f"هل حي {district} مناسب للسكن العائلي؟",
                "🚀 تأثير المشاريع": f"كيف تؤثر المشاريع القريبة على أسعار {property_type} في حي {district}؟"
            }
            st.markdown(f"### {report_titles.get(analysis_type, '')}")
