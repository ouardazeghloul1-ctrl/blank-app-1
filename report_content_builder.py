...
# =========================
def decision_invalid_conditions_block(final_decision):
    """
    قسم مؤسسي هادئ:
    متى لا يكون هذا القرار صحيحًا؟
    """
...
هذا القرار مبني على معطيات واضحة ومتكاملة في لحظة زمنية محددة،
لكنه ليس افتراضًا جامدًا لا يتأثر بالسياق.

قد يصبح هذا القرار غير مناسب إذا ظهرت واحدة أو أكثر من الإشارات التالية:

{chr(10).join(f"• {c}" for c in final_decision.change_triggers)}

في حال ظهور هذه المؤشرات،
لا يعني ذلك أن القرار كان خاطئًا،
بل أن السوق بدأ يغيّر قواعده بهدوء.

في هذه الحالة، التوصية ليست الاندفاع للتصرف،
بل إعادة التقييم بنفس المنهجية التي بُني عليها هذا التقرير.

القرارات الناضجة لا تُلغى عند أول تغير،
بل تُراجع عندما تتغير الفرضية الأساسية التي قامت عليها.
"""
    }
