...
إصدار: 1.0.1 - مع توحيد أسماء الباقات
تاريخ: 2024
"""
...
# ========== نظام البناء الكامل مع توحيد الباقات ==========
def build_complete_report(user_info):
    """بناء التقرير الكامل مع الفلترة حسب الباقة"""
    
    # 🔴 التعديل: توحيد أسماء الباقات (عربي → تقني)
    raw_package = user_info.get("package", "free")
    package = PACKAGE_ALIASES.get(raw_package)
    
    if not package:
        raise ValueError(
            f"نوع الباقة غير مدعوم: {raw_package}. "
            f"الباقات المتاحة: {', '.join(PACKAGE_ALIASES.keys())}"
        )
    
    # 🔒 توحيد قسري لمنع أي تضارب لاحق
    ...
    for i, chapter_func in enumerate(all_chapters_funcs, 1):
        ...
        for block in chapter_blocks:
            if package in block.get("show_in", []):
                ...
                # عد البلوكات القابلة للتعديل بالذكاء الاصطناعي
    ...
    filtered_report["stats"] = {
        ...
        "total_ai_editable": total_ai_editable,
...
# ========== اختبار النظام مع الباقات العربية ==========
if __name__ == "__main__":
    print("🔧 اختبار نظام التقارير مع توحيد أسماء الباقات")
    print("=" * 60)
    
    # اختبار كل الباقات بالعربية
    test_cases = [
        {"package": "مجانية", "نوع_العقار": "شقق سكنية", "المدينة": "الرياض"},
        {"package": "فضية", "نوع_العقار": "فيلا", "المدينة": "جدة"},
        {"package": "ذهبية", "نوع_العقار": "أرض سكنية", "المدينة": "الدمام"},
        {"package": "ماسية", "نوع_العقار": "عمارة سكنية", "المدينة": "مكة"},
        {"package": "ماسية متميزة", "نوع_العقار": "مجمع تجاري", "المدينة": "الخبر"},
        # اختبار بالإنجليزية أيضًا
        {"package": "diamond_plus", "نوع_العقار": "برج سكني", "المدينة": "الظهران"},
    ]
    
    print("\n📊 نتائج اختبار جميع الباقات (عربي + إنجليزي):")
    for test_case in test_cases:
        try:
            report = build_complete_report(test_case)
            print(f"\n✅ {report['package_name']} ({test_case['package']} → {report['package']}):")
    ...
    # اختبار باقة غير صالحة
    print("\n🔍 اختبار باقة غير صالحة:")
    try:
        build_complete_report({"package": "باقة غير موجودة", "نوع_العقار": "شقق", "المدينة": "الرياض"})
    except ValueError as e:
        print(f"✅ نجاح: تم رفض الباقة غير الصالحة - {e}")
    
    print("\n" + "=" * 60)
    print("🎯 توحيد أسماء الباقات تم بنجاح!")
    print(f"   • الباقات المدعومة: {len(PACKAGE_ALIASES)} اسم (عربي + إنجليزي)")
    print("   • الآن يعمل مع: 'مجانية', 'فضية', 'ذهبية', 'ماسية', 'ماسية متميزة'")
    print("   • وأيضًا مع: 'free', 'silver', 'gold', 'diamond', 'diamond_plus'")
    print("\n🚀 النظام جاهز 100% للعمل مع جميع الواجهات!")
