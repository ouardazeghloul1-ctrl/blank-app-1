...
📈 توقعات السوق:
• النمو: {safe_num(market_data.get('معدل_النمو_الشهري', 2.5), '.1f')}% شهرياً
• السيولة: {safe_num(market_data.get('مؤشر_السيولة', 85), '.0f')}%
...
class AIIntelligence:
    ...
    def future_scenarios(self, market_data):
        ...
        scenarios = {
            ...
            'السيناريو_المعتدل': {
                'احتمالية': '50%',
                'التوقع': f"استمرار النمو الحالي {safe_num(base_growth, '.1f')}% مع استقرار السوق",
                'العائد_المتوقع': f"{safe_num(base_return, '.1f')}%",
                'التوصية': "الاستمرار في الخطة الاستثمارية مع مراقبة المؤشرات"
            },
            'السيناريو_المتشائم': {
                'احتمالية': '15%',
                'التوقع': f"تباطؤ مؤقت في النوم بمعدل {safe_num(max(base_growth - 1, 0.5), '.1f')}% مع انخفاض الطلب",
                'العائد_المتوقع': f"{safe_num(max(base_return - 3, 4), '.1f')}%",
