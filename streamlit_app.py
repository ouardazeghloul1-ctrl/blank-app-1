...
with col3:
    الباقات = ["الباقة الأساسية - $99", "الباقة المتقدمة - $199", "الباقة الاحترافية - $399"]
...
def إنشاء_بيانات_سعودية(المدينة, نوع_العقار, عدد_العقارات):
    np.random.seed(42)
    ...
    أسعار_قاعدة = {
        ...
        "خميس مشيط": {"شقة": 90000, "فيلا": 220000, "أرض": 60000, "محل تجاري": 160000, "مكتب": 120000}
    }
...
# إنشاء البيانات
df = إنشاء_بيانات_سعودية(المدينة, نوع_العقار, عدد_العقارات)
...
if "احترافية" in الباقة:
    ...
    with col_ai1:
        ...
        **📈 تنبؤات الأسعار لـ 30 يوم:**
        ...
        # إحصائيات تنبؤية وهمية للعرض
        st.metric("📊 اتجاه السوق", "📈 صاعد", "+5.2%")
        st.metric("🎯 أفضل وقت للشراء", "خلال أسبوعين")
        st.metric("💰 متوسط العائد المتوقع", "8.5%")
    ...
    with col_ai2:
        ...
        **🔮 تحليل الذكاء الاصطناعي:**
        ...
        st.metric("🏆 أفضل منطقة", f"{df.nsmallest(1, 'سعر_المتر')['الحي'].iloc[0]}")
        st.metric("⚠️ مناطق يجب تجنبها", "2 منطقة")
        st.metric("💡 فرص استثمارية", f"{len(df)//10} فرصة")
...
with col_دفع1:
    ...
    **ما ستحصل عليه:**
    ✓ تقرير PDF كامل ({عدد_العقارات} عقار)
    ✓ تحليل مفصل وشامل
...
with st.form("تأكيد_الدفع"):
    ...
    if submitted:
        if email and transaction_id:
            ...
            def create_pdf_report(df, المدينة, نوع_العقار, الباقة, عدد_العقارات):
                pdf = FPDF()
                pdf.add_page()
                ...
                pdf.cell(200, 10, txt=f"عدد العقارات المحللة: {عدد_العقارات:,}", ln=True)
            ...
            # زر تحميل PDF
            try:
                pdf_data = create_pdf_report(df, المدينة, نوع_العقار, الباقة, عدد_العقارات)
                st.download_button(
...
st.markdown("---")
st.markdown("✨ **وردة العقارية 2024** - تحليلات عقارية ذكية بدقة عالية")
