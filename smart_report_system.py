...
class SmartReportSystem:
    ...
    def _create_advanced_roi_analysis(self, real_data):
        ...
        • المتوسط: {safe_num(roi_stats['mean'], '.1f')}%
        • الوسيط: {safe_num(roi_stats['50%'], '.1f')}%
        • أعلى 20%: {safe_num(roi_stats['80%'], '.1f')}%
        • الانحراف المعياري: {safe_num(roi_stats['std'], '.1f')}%
    ...
    def _create_market_trends_analysis(self, market_data):
        ...
        📊 **مؤشرات النمو:**
        • معدل النمو الشهري: {safe_num(growth, '.1f')}%
    ...
    def _create_property_comparison(self, real_data, user_info):
        ...
        if not comparable.empty:
            ...
            • متوسط أسعار السوق: {safe_num(avg_price)} ريال
            • متوسط سعر المتر: {safe_num(avg_psm)} ريال
